...
## Remarks
Lua is compiled into the plugin itself, so lua.dll (or the static plugin) has no external dependencies. Lua is distributed under the MIT license; the attribution is included in NVGT's third party code attributions document.
