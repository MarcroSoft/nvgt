...
Another configuration option worth mentioning is the build.shared_library_excludes property, which is empty by default. NVGT automatically bundles only the shared libraries your game actually needs: the core libraries plus any that the plugins you include register for bundling. If you want to leave out a library that would otherwise be bundled, list partial library names (separated by spaces, commas, or semicolons) in this property. For example, a game that uses neither the phonon spatial audio library nor the screen reader clients could set build.shared_library_excludes = "phonon SAAPI64 nvdaControllerClient64 zdsrapi nvgt_sapi32host" in its configuration file to keep them out of the final product.
