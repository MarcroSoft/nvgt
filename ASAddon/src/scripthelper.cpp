...
std::string ScriptGetExceptionFunctionDecl()
{
	asIScriptContext *ctx = asGetActiveContext();
	if (!ctx)
		return "";
...
std::string ScriptGetExceptionModule()
{
	asIScriptContext *ctx = asGetActiveContext();
	if (!ctx)
		return "";
...
void RegisterExceptionRoutines(asIScriptEngine *engine)
{
	...
	if (strstr(asGetLibraryOptions(), "AS_MAX_PORTABILITY") == 0)
	{
		...
		r = engine->RegisterGlobalFunction("string get_exception_file()", asFUNCTION(ScriptGetExceptionModule), asCALL_CDECL); assert(r >= 0);
	}
	else
	{
